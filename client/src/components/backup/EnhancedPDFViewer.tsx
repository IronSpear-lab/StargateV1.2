          ...
          <div className="p-4">
            {sidebarMode === 'details' && (
              <div>
                ...
                {/* Aktivitet - modern design */}
                <div className="mt-8">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Aktivitet</h3>
                  ...
                  {/* Annotations activity - modern design */}
                  {annotations.length > 0 && (
                    <div className="mb-6 space-y-4">
                      <div className="flex items-center">
                        <div className="h-px flex-1 bg-gray-200 dark:bg-gray-700"></div>
                        <span className="mx-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-900 px-2">Kommentarer</span>
                        <div className="h-px flex-1 bg-gray-200 dark:bg-gray-700"></div>
                      </div>
                        ...
                        {annotations.slice().reverse().map((annotation) => (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div 
                                className="w-3 h-3 mt-1.5 rounded-full flex-shrink-0 ring-2 ring-white dark:ring-gray-700" 
                                style={{ backgroundColor: annotation.color }}
                              />
                              <div className="flex-1 overflow-hidden">
                                {/* Comment badge based on status */}
                                <div className="flex items-center gap-2 mb-1.5">
                                  <Badge 
                                    className={`text-xs ${
                                      annotation.status === 'action_required' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                                      annotation.status === 'resolved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                      annotation.status === 'rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                  ...
                                  <span className="text-xs text-muted-foreground">Sida {annotation.rect.pageNumber}</span>
                                </div>
                                
                                <div className="font-medium text-sm">
                                  {annotation.comment.substring(0, 80)}{annotation.comment.length > 80 ? '...' : ''}
                                </div>
                                
                                <div className="flex items-center text-xs text-muted-foreground mt-2">
                                  <Avatar className="h-5 w-5 mr-1.5 border border-white dark:border-gray-700">
                                    <AvatarImage src={`https://avatar.vercel.sh/${annotation.createdBy}.png`} />
                                    <AvatarFallback className="text-[10px]">{annotation.createdBy.substring(0, 1).toUpperCase()}</AvatarFallback>
                                  </Avatar>
                                  <span className="mr-2 font-medium">{annotation.createdBy}</span>
                  ...
                  {/* Version history activity - modern design */}
                  <div className="mt-6 space-y-4">
                    <div className="flex items-center">
                      <div className="h-px flex-1 bg-gray-200 dark:bg-gray-700"></div>
                      <span className="mx-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-900 px-2">Versioner</span>
                      <div className="h-px flex-1 bg-gray-200 dark:bg-gray-700"></div>
                    </div>
                    ...
                    <div className="space-y-3">
                      {fileVersions.slice().reverse().map((version, i) => (
                        <div 
                          key={version.id} 
                          className={`p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg ${i === 0 ? 'border-blue-200 dark:border-blue-800 shadow-sm' : ''}`}
                        >
                          <div className="flex items-start gap-3">
                            <div className={`h-8 w-8 rounded-full ${i === 0 ? 'bg-blue-100 dark:bg-blue-900/40' : 'bg-gray-100 dark:bg-gray-800'} flex items-center justify-center flex-shrink-0`}>
                              <FileCheck className={`h-4 w-4 ${i === 0 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`} />
                            </div>
                            
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center">
                                  <span className="font-medium text-sm">{version.description || `Version ${version.versionNumber}`}</span>
                                  {i === 0 && (
                                    <Badge className="ml-2 bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 border-none">Aktuell</Badge>
                                  )}
                                </div>
                              ...
                              <div className="flex items-center text-xs text-muted-foreground">
                                <Avatar className="h-4 w-4 mr-1.5">
                                  ...
                                  <AvatarFallback className="text-[8px]">{version.uploadedBy?.substring(0, 1).toUpperCase()}</AvatarFallback>
                                </Avatar>
                                <span className="font-medium mr-1">{version.uploadedBy}</span>
                                <span>uppladdad {formatDate(version.uploaded)}</span>
                              </div>
                            </div>
                          </div>
                      
                          {version.id === fileVersions[fileVersions.length - 1]?.id && (
                            <div className="text-xs mt-2 px-2 py-1 bg-gray-50 dark:bg-gray-700/50 rounded text-muted-foreground">
                              <History className="h-3 w-3 inline-block mr-1" />
                              Uppdaterade ny version
                            </div>
                          )}
                          
                          {i === 1 && version.id !== activeVersionId && (
                            <div className="text-xs mt-2 px-2 py-1 bg-blue-50 dark:bg-blue-900/20 rounded text-blue-700 dark:text-blue-400">
